...
Processes SHST extraction data to network standard, and conflates with OSM attributes.
...
ROOT_DATA_DIR = 'C:\\Users\\{}\\Box\\Modeling and Surveys\\Development\\Travel Model Two ' \
    ...
    os.getenv('USERNAME'))
OSM_EXTRACT_DIR = os.path.join(ROOT_DATA_DIR, 'external', 'step2_osmnx_extraction')
OSM_LINK_FILE = os.path.join(OSM_EXTRACT_DIR, 'link.geojson')
OSM_NODE_FILE = os.path.join(OSM_EXTRACT_DIR, 'node.geojson')
SHST_EXTRACT_DIR = os.path.join(ROOT_DATA_DIR, 'external', 'step1_shst_extraction')

HIGHWAY_TO_ROADWAY_CROSSWALK_FILE = os.path.join(ROOT_DATA_DIR, 'lookups', 'highway_to_roadway.csv')
NETWORK_TYPE_LOOKUP_FILE = os.path.join(ROOT_DATA_DIR, 'lookups', 'network_type_indicator.csv')

OUTPUT_DIR = os.path.join(ROOT_DATA_DIR, 'interim', 'step3_join_shst_extraction_with_osm')
...
with open(os.path.join(OUTPUT_DIR, 'step3_shape.geojson'), "w") as f:
...
with open(os.path.join(OUTPUT_DIR, 'step3_link.json'), 'w') as f:
    f.write(out)
...
with open(os.path.join(OUTPUT_DIR, 'step3_node.geojson'), "w") as f:
