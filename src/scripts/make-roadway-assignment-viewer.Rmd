...
assigned_csv_prefix <- paste0(interim_dir, "loaded_net/new_net/", version, "/csv/", "load")
legacy_assigned_csv_prefix <- paste0(interim_dir, "loaded_net/mtc_net/csv/", "load")

pems_match_file_name <- paste0(interim_dir, "mtc/pems_conflation_result.geojson")
...
for (period in c("AM", "MD", "PM")) {
  ...
  df <- read_csv(filename, col_types = cols(.default = col_double(),
                                            ...
                                            "SHSTGEOMETRYID=032" = col_character(),
...
output_sf <- links_df %>%
  ...
  select(shstGeometryId, shstReferenceId, lanes, model_link_id, name, county) %>%
