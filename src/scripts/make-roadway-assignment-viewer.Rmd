...
output_sf <- links_df %>%
  filter(drive_access == 1) %>%
  select(shstGeometryId, shstReferenceId, lanes, model_link_id, county) %>%
...
# Debug
```{r}
bridge_link_ids <- c(8063198, 8063199, 3315451, 3315452)
df <- loaded_df %>%
  filter(model_link_id %in% bridge_link_ids)
