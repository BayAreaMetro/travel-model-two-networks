...
output_pedestrian_sf <- links_df %>%
  filter(walk_access == 1) %>%
  filter(county %in% c("San Francisco", "Alameda", "Contra Costa")) %>%
  select(model_link_id, shstReferenceId, shstGeometryId, name, length) %>%
  left_join(., shapes_sf, by = c("shstGeometryId")) 
...
st_write(output_pedestrian_sf, output_pedestrian_file_name, delete_dsn = TRUE)
```
